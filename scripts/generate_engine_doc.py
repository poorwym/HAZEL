...
def create_file_tree(startpath, output_file):
    output_file.write("# Engine 项目文件结构\n")
...
def write_source_files(startpath, output_file):
    output_file.write("\n\n# 源代码文件内容\n\n")
    ...
    for root, dirs, files in os.walk(startpath):
        ...
        for f in files:
            if f.endswith(('.cpp', '.h', '.hpp', '.inl', '.txt', '.lua')):
...
def main():
    ...
    output_path = os.path.join(script_dir, 'engine_documentation.md')
    
    with open(output_path, 'w', encoding='utf-8') as f:
        create_file_tree(engine_dir, f)
        write_source_files(engine_dir, f)
